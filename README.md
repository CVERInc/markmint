...
- **Gradient fills** per color group (2 stops + angle + presets, one unified ramp)
...
**Export**
- **SVG** (clean, optimized)
- **Raster** — PNG / WebP / JPG at 128–1024px or source resolution
- **Icon pack** — one click produces a zip with a multi-resolution
  `favicon.ico` (16/32/48), `favicon-16/32.png`, `apple-touch-icon.png`,
  `icon-192/512.png`, a safe-area `maskable-512.png`, `site.webmanifest`, and a
  paste-in `<head>` snippet
...
- [ ] Undo / redo history (Cmd/Ctrl+Z)
- [ ] One-click background removal (drop the dominant border color)
- [ ] Advanced trace controls — expose VTracer knobs (corner/splice thresholds, …)
- [ ] Paste from clipboard + bundled sample logos
- [ ] PWA (offline / installable) + Open Graph cards
...
- 色グループごとの **グラデーション**（2 ストップ＋角度＋プリセット）
...
- [ ] 取り消し / やり直し（Cmd/Ctrl+Z）
- [ ] ワンクリック背景除去（支配的な縁の色を削除）
- [ ] 高度なトレース設定（VTracer の各パラメータを公開）
- [ ] クリップボードからの貼り付け＋サンプルロゴ同梱
- [ ] PWA（オフライン / インストール可能）＋ OGP カード
